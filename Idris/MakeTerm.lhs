...
>               mif opt ctxt acc' using' ui uo' ds
> mif opt ctxt acc using ui uo ((DoUsing bind ret decls):ds)
...
>                              Left err -> error (show err)
>                     in ui { uibind = (bfull, bimpl),
>                             uireturn = (rfull, rimpl) }
> mif opt ctxt acc using ui uo ((Idiom pure ap decls):ds)
>         = let (acc', uo') = (mif opt ctxt acc using ui' uo decls) in
>             mif opt ctxt acc' using ui uo' ds
...
>                              Left err -> error (show err)
>                     in ui { uipure = (pfull, pureImpl),
>                             uiapply = (afull, apImpl) }
> mif opt ctxt acc using ui uo ((DSL var lam decls):ds)
>         = let (acc', uo') = (mif opt ctxt acc using ui' uo decls) in
>             mif opt ctxt acc' using ui uo' ds
...
>                              Left err -> error (show err)
>                     in ui { uivar = (Just vfull, varImpl),
>                             uilam = (Just lfull, lamImpl) }
